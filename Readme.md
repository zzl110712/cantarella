...
- **PNG 档位压缩**：`-q` 映射到调色板档位（256 / 16 / 4 / 2 色台阶，档内用抖动强度微调）；`-q 81+` 走真彩无损重编码（zlib 顶格 + 自适应行过滤）
...
| 选项                     | 说明                                                                                                       | 默认值                |
| ------------------------ | ---------------------------------------------------------------------------------------------------------- | --------------------- |
| `-q, --quality <number>` | 压缩质量 1-100（png 映射为调色板颜色数）                                                                    | `80`                  |
...
PNG 没有平滑的有损 quality，`-q` 被映射到调色板档位上——颜色数决定体积台阶（索引位宽 8/4/2/1bpp），档内用抖动强度微调画质与字节：

| quality | 档位 | 效果 |
| ------- | ---- | ---- |
| 81-100  | 真彩无损重编码 | zlib 顶格压缩 + 自适应行过滤，像素级保真（编码慢 4-6 倍） |
| 70-80   | 256 色量化 + 全抖动 | 近无损，默认 `-q 80` 即此档 |
| 40-69   | 16 色量化 | 体积第一次断崖；档内抖动随 quality 递减 |
| 10-39   | 4 色量化 | 再腰斩，观感受损明显 |
| 1-9     | 黑白二值 | 1bpp 极限 |

```bash
cantarella compress ./images -q 80    # 默认档：256 色近无损量化
cantarella compress ./images -q 60    # 16 色：截图 / UI 图形的甜点档
cantarella compress ./images -q 85    # 无损真彩：像素级保真
cantarella compress ./images -s       # smart 模式：无损 png 与无损 webp 同场比价
cantarella compress ./images -f webp  # 照片类收益最大
```

- **截图 / UI 图形**（大面积纯色）：`-q 60` 效果极好
- **照片**存 PNG：默认 `-q 80` 的 256 色 + 全抖动已可放心用；色带敏感就用 `-q 85` 走无损
...
- ✨ **重构** PNG 的 `-q` 语义为档位表：旧公式"quality 越低压缩等级越高"存在语义倒置（`-q 100` 会算出 cl0 = 完全不压缩），且 `quality <= 60` 才开 palette 的门槛太高；现在 quality 直接踩在 PNG 的位深台阶上（256 / 16 / 4 / 2 色），档内用抖动强度微调画质与字节
- ✨ **新增** `-q 81+` 真彩无损重编码：`compressionLevel` 恒定 9（zlib 无损、压多狠都不损画质，没有理由不顶格）+ `adaptiveFiltering` 自适应行过滤，实测照片类比旧写法再小 20-40%（代价是编码慢 4-6 倍，与 avif 同量级的取舍）
- 💡 默认 `-q 80` 从"无损重编码"变为"256 色近无损量化 + 全抖动"：已优化过的 PNG 默认档就能压动，照片类典型节省 80% 上下
